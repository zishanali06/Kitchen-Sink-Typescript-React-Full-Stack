import * as React from 'react';

export interface BloghomeProps {

}

export interface BloghomeState {

}

class Bloghome extends React.Component<BloghomeProps, BloghomeState> {
    constructor(props: BloghomeProps) {
        super(props);
        this.state = {};
    }
    render() {
        return (
            <main className="container">
                <section className="row d-flex justify-content-center align-items-center bg1 text-center text-white">
                    <h1 className="col-12 mt-5 tshadow">JavaScript done right.</h1>
                    <button className="btm btn-outline-danger mb-5 mr-3">I need a website</button>
                    <button className="btm btn-outline-danger mb-5 ml-3">I'm looking to hire</button>
                </section>
                <section className="row">
                    <h1>testing 123</h1>
                    <p>askdfasdf</p>
                    <p>askdfasdf</p>
                    <p>askdfasdf</p>
                    <p>askdfasdf</p>
                </section>
                <section className="row">
                    <h1>testing 123</h1>
                    <p>askdfasdf</p>
                    <br />
                    <p>askdfasdf</p>
                    <br />
                    <p>askdfasdf</p>
                    <br />
                    <p>askdfasdf</p>
                    <p>askdfasdf</p>
                    <p>askdfasdf</p>
                    <p>askdfasdf</p>
                </section>
                <section className="row">
                    <h1>testing 123</h1>
                    <p>askdfasdf</p>
                    <p>askdfasdf</p>
                    <p>askdfasdf</p>
                    <p>askdfasdf</p>
                    <br />
                    <p>askdfasdf</p>
                    <br />
                    <p>askdfasdf</p>
                    <p>askdfasdf</p></section>
                <section className="row">
                    <h1>testing 123</h1>
                    <p>askdfasdf</p>
                    <p>askdfasdf</p>
                    <br />
                    <p>askdfasdf</p>
                    <br />
                    <p>askdfasdf</p>
                    <br />
                    <p>askdfasdf</p>
                    <br />
                    <p>askdfasdf</p>
                    <p>askdfasdf</p></section>
                    <section className="row">
                    <h1>testing 123</h1>
                    <p>askdfasdf</p>
                    <p>askdfasdf</p>
                    <br />
                    <p>askdfasdf</p>
                    <br />
                    <p>askdfasdf</p>
                    <br />
                    <p>askdfasdf</p>
                    <br />
                    <p>askdfasdf</p>
                    <p>askdfasdf</p></section>
                    <section className="row">
                    <h1>testing 123</h1>
                    <p>askdfasdf</p>
                    <p>askdfasdf</p>
                    <br />
                    <p>askdfasdf</p>
                    <br />
                    <p>askdfasdf</p>
                    <br />
                    <p>askdfasdf</p>
                    <br />
                    <p>askdfasdf</p>
                    <p>askdfasdf</p></section>
                    <section className="row">
                    <h1>testing 123</h1>
                    <p>askdfasdf</p>
                    <p>askdfasdf</p>
                    <br />
                    <p>askdfasdf</p>
                    <br />
                    <p>askdfasdf</p>
                    <br />
                    <p>askdfasdf</p>
                    <br />
                    <p>askdfasdf</p>
                    <p>askdfasdf</p></section>
                    <section className="row">
                    <h1>testing 123</h1>
                    <p>askdfasdf</p>
                    <p>askdfasdf</p>
                    <br />
                    <p>askdfasdf</p>
                    <br />
                    <p>askdfasdf</p>
                    <br />
                    <p>askdfasdf</p>
                    <br />
                    <p>askdfasdf</p>
                    <p>askdfasdf</p></section>
                    <section className="row">
                    <h1>testing 123</h1>
                    <p>askdfasdf</p>
                    <p>askdfasdf</p>
                    <br />
                    <p>askdfasdf</p>
                    <br />
                    <p>askdfasdf</p>
                    <br />
                    <p>askdfasdf</p>
                    <br />
                    <p>askdfasdf</p>
                    <p>askdfasdf</p></section>
                    <section className="row">
                    <h1>testing 123</h1>
                    <p>askdfasdf</p>
                    <p>askdfasdf</p>
                    <br />
                    <p>askdfasdf</p>
                    <br />
                    <p>askdfasdf</p>
                    <br />
                    <p>askdfasdf</p>
                    <br />
                    <p>askdfasdf</p>
                    <p>askdfasdf</p></section>
                    <section className="row">
                    <h1>testing 123</h1>
                    <p>askdfasdf</p>
                    <p>askdfasdf</p>
                    <br />
                    <p>askdfasdf</p>
                    <br />
                    <p>askdfasdf</p>
                    <br />
                    <p>askdfasdf</p>
                    <br />
                    <p>askdfasdf</p>
                    <p>askdfasdf</p></section>
                    <section className="row">
                    <h1>testing 123</h1>
                    <p>askdfasdf</p>
                    <p>askdfasdf</p>
                    <br />
                    <p>askdfasdf</p>
                    <br />
                    <p>askdfasdf</p>
                    <br />
                    <p>askdfasdf</p>
                    <br />
                    <p>askdfasdf</p>
                    <p>askdfasdf</p></section>
                    <section className="row">
                    <h1>testing 123</h1>
                    <p>askdfasdf</p>
                    <p>askdfasdf</p>
                    <br />
                    <p>askdfasdf</p>
                    <br />
                    <p>askdfasdf</p>
                    <br />
                    <p>askdfasdf</p>
                    <br />
                    <p>askdfasdf</p>
                    <p>askdfasdf</p></section>
                    <section className="row">
                    <h1>testing 123</h1>
                    <p>askdfasdf</p>
                    <p>askdfasdf</p>
                    <br />
                    <p>askdfasdf</p>
                    <br />
                    <p>askdfasdf</p>
                    <br />
                    <p>askdfasdf</p>
                    <br />
                    <p>askdfasdf</p>
                    <p>askdfasdf</p></section>
                    <section className="row">
                    <h1>testing 123</h1>
                    <p>askdfasdf</p>
                    <p>askdfasdf</p>
                    <br />
                    <p>askdfasdf</p>
                    <br />
                    <p>askdfasdf</p>
                    <br />
                    <p>askdfasdf</p>
                    <br />
                    <p>askdfasdf</p>
                    <p>askdfasdf</p></section>
                    <section className="row">
                    <h1>testing 123</h1>
                    <p>askdfasdf</p>
                    <p>askdfasdf</p>
                    <br />
                    <p>askdfasdf</p>
                    <br />
                    <p>askdfasdf</p>
                    <br />
                    <p>askdfasdf</p>
                    <br />
                    <p>askdfasdf</p>
                    <p>askdfasdf</p></section>
                    <section className="row">
                    <h1>testing 123</h1>
                    <p>askdfasdf</p>
                    <p>askdfasdf</p>
                    <br />
                    <p>askdfasdf</p>
                    <br />
                    <p>askdfasdf</p>
                    <br />
                    <p>askdfasdf</p>
                    <br />
                    <p>askdfasdf</p>
                    <p>askdfasdf</p></section>
                    <section className="row">
                    <h1>testing 123</h1>
                    <p>askdfasdf</p>
                    <p>askdfasdf</p>
                    <br />
                    <p>askdfasdf</p>
                    <br />
                    <p>askdfasdf</p>
                    <br />
                    <p>askdfasdf</p>
                    <br />
                    <p>askdfasdf</p>
                    <p>askdfasdf</p></section>
                    <section className="row">
                    <h1>testing 123</h1>
                    <p>askdfasdf</p>
                    <p>askdfasdf</p>
                    <br />
                    <p>askdfasdf</p>
                    <br />
                    <p>askdfasdf</p>
                    <br />
                    <p>askdfasdf</p>
                    <br />
                    <p>askdfasdf</p>
                    <p>askdfasdf</p></section>
                    <section className="row">
                    <h1>testing 123</h1>
                    <p>askdfasdf</p>
                    <p>askdfasdf</p>
                    <br />
                    <p>askdfasdf</p>
                    <br />
                    <p>askdfasdf</p>
                    <br />
                    <p>askdfasdf</p>
                    <br />
                    <p>askdfasdf</p>
                    <p>askdfasdf</p></section>
                    <section className="row">
                    <h1>testing 123</h1>
                    <p>askdfasdf</p>
                    <p>askdfasdf</p>
                    <br />
                    <p>askdfasdf</p>
                    <br />
                    <p>askdfasdf</p>
                    <br />
                    <p>askdfasdf</p>
                    <br />
                    <p>askdfasdf</p>
                    <p>askdfasdf</p></section>
                    <section className="row">
                    <h1>testing 123</h1>
                    <p>askdfasdf</p>
                    <p>askdfasdf</p>
                    <br />
                    <p>askdfasdf</p>
                    <br />
                    <p>askdfasdf</p>
                    <br />
                    <p>askdfasdf</p>
                    <br />
                    <p>askdfasdf</p>
                    <p>askdfasdf</p></section>

            </main>
        );
    }
}

export default Bloghome;